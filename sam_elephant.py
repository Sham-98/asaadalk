...
output_csv = "SAM_elephant_results.csv"
...
for image_path in full_image_paths:
    ...
    output = processor.set_text_prompt(state=inference_state, prompt="Savana elephant") 
    masks, boxes, scores = output["masks"], output["boxes"], output["scores"]
    # Close image to free memory immediately
    image.close()

    detection = len(scores)
    elephant_savanna = "elephant_savanna" if detection > 0 else "other"

    image_name = os.path.basename(image_path)
    print("image_path:", image_name)
    print("Species category:", elephant_savanna)
    ...
    row = pd.DataFrame([{
        ...
        "Detection": detection,
        "Species category": elephant_savanna,
...
print("SAM_elephant_results.csv written.")
print("working2!")
