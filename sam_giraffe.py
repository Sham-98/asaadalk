...
thresholds = 0.9
output_csv = "SAM_giraffe_results.csv"
...
for image_path in full_image_paths:
    ...
    output = processor.set_text_prompt(state=inference_state, prompt="Reticulated giraffe") 
    masks, boxes, scores = output["masks"], output["boxes"], output["scores"]
    # Close image to free memory immediately
    image.close()

    detection = len(scores)
    giraffe_reticulated = "giraffe_reticulated" if detection > 0 else "other"

    image_name = os.path.basename(image_path)
    print("image_path:", image_name)
    print("Species category:", giraffe_reticulated)
    ...
    row = pd.DataFrame([{
        ...
        "Detection": detection,
        "Species category": giraffe_reticulated,
...
print("SAM_giraffe_results.csv written.")
print("working2!")
