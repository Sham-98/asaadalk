...
for image_path in full_image_paths:
    ...
    output = processor.set_text_prompt(state=inference_state, prompt="Plains zebra") 
    masks, boxes, scores = output["masks"], output["boxes"], output["scores"]
    # Close image to free memory immediately
    image.close()

    detection = len(scores)
    zebra_plains = "zebra_plains" if detection > 0 else "other"

    image_name = os.path.basename(image_path)
    print("image_path:", image_name)
    print("Species category:", zebra_plains)
...
print("SAM_plains_results.csv written.")
print("working2!")
